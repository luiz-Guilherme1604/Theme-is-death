...
function updateGameArea() {
  ...
  if (!gameStarted) {
    ...
    if(cart.tips){
      if(lvl==0){
        writeTxt(ctx, 1, font,"WHITE","Pule a lacuna e corra sobre o botão para chegar ao portal!", 200, 120);
      } else if(lvl==1){
        writeTxt(ctx, 1, font,"WHITE","Não tenha medo do ceifador! Morrer cria uma plataforma de alma!", 230, 80);
        writeTxt(ctx, 1, font,"WHITE","Rebobine sua alma para posições anteriores com [E]", 230, 100);
      } else if(lvl==2){
        writeTxt(ctx, 1, font,"WHITE","Você pode pular da sua alma!", 240, 80);
        writeTxt(ctx, 1, font,"WHITE","Evite picos e limpe bordas altas", 240, 100);
      } else if(lvl==3){
        writeTxt(ctx, 1, font,"WHITE","Aperte o botão e recupere sua alma, segure [E]", 230, 80);
        writeTxt(ctx, 1, font,"WHITE","Reutilize sua alma no segundo conjunto de espinhos!", 230, 100);
      } else if(lvl==4){
        writeTxt(ctx, 1, font,"WHITE","Corra!!", 280, 120);
      } else if(lvl==5){
        writeTxt(ctx, 1, font,"WHITE","Os pontos de verificação permitem que você recupere sua alma e siga em frente!", 230, 100);
      } else if(lvl==10){
        font = "30px Verdana";
        writeTxt(ctx, 1, font,"WHITE","Soul Jumper Concluído!", 230, 100);
      }
    }
